import { NgModule } from '@angular/core';
import { PreloadAllModules, RouterModule, Routes } from '@angular/router';

const routes: Routes = [
  {
    path: '',
    redirectTo: 'folder/Inbox',
    pathMatch: 'full'
  },
  {
    path: 'folder/:id',
    loadChildren: () => import('./folder/folder.module').then( m => m.FolderPageModule)
  },
  {
    path: 'crearcliente',
    loadChildren: () => import('./crearcliente/crearcliente.module').then( m => m.CrearclientePageModule)
  },
  {
    path: 'eliminarcliente',
    loadChildren: () => import('./eliminarcliente/eliminarcliente.module').then( m => m.EliminarclientePageModule)
  },
  {
    path: 'libro',
    loadChildren: () => import('./libro/libro.module').then( m => m.LibroPageModule)
  },
  {
    path: 'agregarlibro',
    loadChildren: () => import('./agregarlibro/agregarlibro.module').then( m => m.AgregarlibroPageModule)
  },
  {
    path: 'editorial',
    loadChildren: () => import('./editorial/editorial.module').then( m => m.EditorialPageModule)
  },
  {
    path: 'agregareditorial',
    loadChildren: () => import('./agregareditorial/agregareditorial.module').then( m => m.AgregareditorialPageModule)
  }
];

@NgModule({
  imports: [
    RouterModule.forRoot(routes, { preloadingStrategy: PreloadAllModules })
  ],
  exports: [RouterModule]
})
export class AppRoutingModule {}
